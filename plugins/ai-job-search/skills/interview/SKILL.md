---
name: interview
description: "Prepare for an Interview on a Tracked Application. Use when the user runs /interview."
...
Match the ready-made STAR examples in `profile/star.md` to the likely questions using their "Use for" tags. Then:
- For likely questions **no existing STAR example covers**, draft a new STAR answer grounded strictly in facts from `profile/candidate.md` - profile facts arranged into S/T/A/R, not embellished. Include these drafts in the prep pack; offer to append them to `profile/star.md` only if the user explicitly approves.
...
Calibrate feedback against `profile/behavioral.md`: coach toward the user's natural register, not a generic ideal - the same voice-consistency rule the `/apply` reviewer applies to cover letters.
...
If Step 3 drafted new STAR answers the user approved for keeps, remind them those were appended to `profile/star.md` (or offer again if they deferred).
