...
```bash
python3 ${CLAUDE_SKILL_DIR}/../job-tools/scripts/rank_state.py candidates --limit 10          # add --all / --focus "<text>" per Step 0
```
...
6. **Expiry sweep over already-ranked entries.** Before presenting, check the stored `deadline` of every `ranked` entry this run did not re-score:
   ...
   ```bash
   python3 ${CLAUDE_SKILL_DIR}/../job-tools/scripts/rank_state.py sweep --write --exclude "<keys scored this run, comma-separated>"
   ```
...
```bash
python3 ${CLAUDE_SKILL_DIR}/../job-tools/scripts/rank_state.py apply --results "<path to that temporary file>"
```
...
5. **State moves through the tool, not the context.** `seen_jobs.json` is read, swept and written by `${CLAUDE_SKILL_DIR}/../job-tools/scripts/rank_state.py`. It is never read into the conversation to be filtered by eye, and never re-emitted to be updated by hand: both cost the whole backlog per run and grow for the life of the workspace.
