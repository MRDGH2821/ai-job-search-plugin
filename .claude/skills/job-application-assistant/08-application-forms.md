...
**Every claim in a form field must already be defensible from the same sources the CV and cover letter are grounded against** — the union of `profile/candidate.md` and the master CV (`cv/main_example.tex`), with a claim grounded if EITHER supports it. The interviewer reads the form alongside the CV. A form field is not a place to introduce new claims, inflate scope, or fill space — it is a place to *select* from what is already true and arrange it for the question asked.
...
- [ ] Every factual claim traces to `profile/candidate.md` or the master CV (`cv/main_example.tex`)
